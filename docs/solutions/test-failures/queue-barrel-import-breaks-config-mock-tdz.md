...
- [`test-failures/nestjs-config-joi-validation-dotenv-conflict-2026-05-06.md`](nestjs-config-joi-validation-dotenv-conflict-2026-05-06.md) — closest sibling: a spec crashes at module-import time because importing `@hsm/config` triggers Joi validation of `envs` before any test runs. Same root-cause family (import-time eager env evaluation breaking specs); different reach (direct config import) and fix (env-shim in `test-setup.ts`).
- [`runtime-errors/typeorm-entity-circular-import-silent-drop-2026-05-04.md`](../runtime-errors/typeorm-entity-circular-import-silent-drop-2026-05-04.md) — same *family* (barrel re-export causing wrong-time module evaluation), different symptom (entity barrel evaluates to `[]` → runtime DI crash). The general rule "import from the side-effect-free leaf path, not the barrel" applies to both.
...
- `docs/solutions/developer-experience/nestjs-unit-test-mocking-patterns-2026-05-06.md` — **stale example**: its `getQueueToken` snippet imports `QueueEnum` from the `@hsm/queue` barrel, the exact import this learning shows is hazardous under a mocked `@hsm/config`. Worth updating to the leaf path.
